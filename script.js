...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (codeInput) {
    ...
    codeInput.addEventListener("paste", () => {
      ...
      setTimeout(() => updateLineNumbers(true), 0);
    });
  ...
  function buildPrompt(code, mode, instructions) {
    ...
    if (mode === "nl-to-code") {
      ...
      const systemPrompt =
        ...
        "IMPORTANT: Return ONLY the code without any JSON wrapper, markdown formatting, or explanations. " +
        "Just output the raw code that can be directly copied and used.";
      ...
      let userPrompt =
        ...
        "Provide ONLY the code - no explanations, no markdown, no JSON.";
    ...
    const systemPrompt =
      ...
      modeConfig.title +
      ".\n\nIMPORTANT: Return ONLY the improved code without any JSON wrapper, markdown formatting, or explanations. " +
      "Just output the raw code that can be directly copied and used.\n\nFocus: " +
    ...
    let userPrompt =
      ...
      "\nTASK: " +
      modeConfig.task +
      "\n\nProvide ONLY the improved code - no explanations, no markdown, no JSON. Just the code ready to copy and paste.\n\nCODE:\n" +
      code;
  ...
  if (copyBtn) {
    copyBtn.addEventListener("click", () => {
      if (outputCode) navigator.clipboard.writeText(outputCode.textContent);
      copyBtn.textContent = "✓";
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener("click", () => {
      if (outputCode)
        downloadFile(outputCode.textContent, "improved-code.txt", "text/plain");
    });
  }
  ...
  function toggleDiffView() {
    if (!currentCode || !currentImprovedCode) {
      showMessage("No code to compare. Run an improvement first!", "error");
      return;
    }
    ...
    if (isDiffViewActive) {
      if (outputCode) outputCode.style.display = "none";
      if (diffContainer) diffContainer.style.display = "block";
      renderDiffView();
      if (diffBtn) diffBtn.textContent = "📝"; // Switch icon to indicate code view
    } else {
      if (outputCode) outputCode.style.display = "block";
      if (diffContainer) diffContainer.style.display = "none";
      if (diffBtn) diffBtn.textContent = "🔄"; // Switch back to diff icon
    }
  }

  function renderDiffView() {
    if (!diffOriginal || !diffImproved) return;

    const originalLines = currentCode.split("\n");
    const improvedLines = currentImprovedCode.split("\n");

    // Generate diff
    const diff = computeDiff(originalLines, improvedLines);

    // Render original pane
    let originalHtml = "";
    let improvedHtml = "";
    let lineNumOriginal = 0;
    let lineNumImproved = 0;

    diff.forEach((item) => {
      if (item.type === "removed") {
        lineNumOriginal++;
        originalHtml +=
          '<span class="diff-line removed"><span class="diff-line-number">' +
          lineNumOriginal +
          "</span>" +
        ...
        lineNumImproved++;
        improvedHtml +=
          '<span class="diff-line added"><span class="diff-line-number">' +
          lineNumImproved +
          "</span>" +
          escapeHtml(item.value) +
          "</span>\n";
        ...
        lineNumImproved++;
        originalHtml +=
          '<span class="diff-line unchanged"><span class="diff-line-number">' +
          lineNumOriginal +
          "</span>" +
          escapeHtml(item.value) +
          "</span>\n";
    ...
    diffImproved.innerHTML = improvedHtml;
  }
  ...
  function computeDiff(original, improved) {
    ...
    const lcs = computeLCS(original, improved);

    let origIdx = 0;
    let impIdx = 0;
    let lcsIdx = 0;

    while (origIdx < original.length || impIdx < improved.length) {
      if (
        lcsIdx < lcs.length &&
        origIdx < original.length &&
        impIdx < improved.length &&
        original[origIdx] === lcs[lcsIdx] &&
        improved[impIdx] === lcs[lcsIdx]
      ) {
        // Common line
        result.push({ type: "unchanged", value: original[origIdx] });
        origIdx++;
        impIdx++;
        lcsIdx++;
      } else if (
        ...
        // Removed line
        result.push({ type: "removed", value: original[origIdx] });
        origIdx++;
      } else if (impIdx < improved.length) {
        // Added line
        result.push({ type: "added", value: improved[impIdx] });
        impIdx++;
      }
    }
  ...
  function updateStats() {
    ...
    localStorage.setItem("aicodeimp_total", totalImprovements.toString());
    localStorage.setItem("aicodeimp_time", totalTimeSaved.toString());
  ...
  function showMessage(message, type) {
    ...
    messageDiv.style.cssText =
      "position:fixed;top:20px;left:50%;transform:translateX(-50%);padding:15px 25px;border-radius:10px;z-index:9999;background:" +
      (type === "error" ? "rgba(255,0,100,0.9)" : "rgba(0,255,150,0.9)") +
      ";color:white;font-family:'Rajdhani',sans-serif;font-weight:bold;box-shadow:0 5px 30px rgba(0,0,0,0.5);";
